...
namespace IT_asserts_Claim.Services.Implementations
{
    public class PurchaseAttachmentService : IPurchaseAttachmentService
    {
        ...
        public async Task<List<PurchaseAttachmentDto>> GetAllByPurchaseIdAsync(Guid purchaseId)
        {
            var attachments = await _attachmentRepository.GetAllByPurchaseIdAsync(purchaseId);
            return _mapper.Map<List<PurchaseAttachmentDto>>(attachments);
        }

        public async Task<List<PurchaseAttachmentDto>> UploadAsync(Guid purchaseId, List<IFormFile> files)
        {
